...
const langCode: LangCode = langUI === 'zh-CN' ? 'zh-CN' : langUI === 'zh-TW' || langUI === 'zh-HK' ? 'zh-TW' : 'en'
...
function _getDefaultConfig() {
  return {
    ...
    pdfBlacklist: [
      ...
      ['^(http|https)://[^/]*?googleusercontent\\.com(/.*)?$', '*://*.googleusercontent.com/*'],
      ['^(http|https)://sh-download\\.weiyun\\.com(/.*)?$', '*://sh-download.weiyun.com/*'],
    ] as [string, string][],
    ...
    autopron: {
      ...
      en: {
        dict: '' as DictID | '',
        list: ['bing', 'cambridge', 'cobuild', 'eudic', 'longman', 'macmillan', 'lexico', 'urban', 'websterlearner', 'youdao'] as DictID[],
        accent: 'uk' as 'us' | 'uk',
      },
      machine: {
        dict: '' as DictID | '',
